...
pub fn parse_single_static_segment_test() {
  pattern_parser.parse_pattern("/users")
  |> should.equal([Literal("users")])
}

pub fn parse_multiple_static_segments_test() {
  pattern_parser.parse_pattern("/api/v1/users")
...
pub fn parse_single_param_test() {
  pattern_parser.parse_pattern("/users/:id")
  |> should.equal([Literal("users"), Param("id")])
}

pub fn parse_multiple_params_test() {
  pattern_parser.parse_pattern("/users/:user_id/posts/:post_id")
  |> should.equal([
    Literal("users"),
    Param("user_id"),
    Literal("posts"),
...
pub fn parse_param_only_test() {
  pattern_parser.parse_pattern("/:id")
...
pub fn parse_single_extension_test() {
  pattern_parser.parse_pattern("/css/*.css")
  |> should.equal([Literal("css"), ExtensionPattern(["css"])])
}

pub fn parse_brace_extensions_test() {
  pattern_parser.parse_pattern("/images/*.{jpg,png,gif}")
...
pub fn parse_complex_pattern_with_all_types_test() {
  pattern_parser.parse_pattern("/api/:version/users/:id/files/**path")
  |> should.equal([
    Literal("api"),
    Param("version"),
    Literal("users"),
...
pub fn parse_multiple_wildcards_test() {
  pattern_parser.parse_pattern("/*/files/*name")
  |> should.equal([
    SingleWildcard(None),
    Literal("files"),
...
pub fn parse_extension_in_middle_test() {
  pattern_parser.parse_pattern("/photos/**/*.{jpg,png}")
  |> should.equal([
    Literal("photos"),
    MultiWildcard(None),
...
pub fn parse_multiple_slashes_test() {
  pattern_parser.parse_pattern("///users///posts///")
