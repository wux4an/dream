...
pub fn start_registers_process_with_name_test() {
  ...
  case dream_singleton.start(name, initial_state, handle_counter_message) {
    Ok(_pid) -> {
      dream_singleton.is_running(name) |> should.be_true()
    }
    Error(_) -> should.fail()
  }
}
...
pub fn call_synchronously_returns_reply_test() {
  ...
  case dream_singleton.start(name, initial_state, handle_counter_message) {
    Ok(_pid) -> {
      case dream_singleton.call(name, GetCount, 1000) {
...
pub fn cast_asynchronously_updates_state_test() {
  ...
  case dream_singleton.start(name, initial_state, handle_counter_message) {
    Ok(_pid) -> {
      // Cast increment
      case dream_singleton.cast(name, Increment) {
        Ok(_) -> {
...
pub fn state_persists_across_multiple_messages_test() {
  ...
  case dream_singleton.start(name, initial_state, handle_counter_message) {
    Ok(_pid) -> {
      ...
      case dream_singleton.call(name, Increment, 1000) {
        Ok(_) -> {
          case dream_singleton.call(name, Increment, 1000) {
            Ok(_) -> {
              case dream_singleton.call(name, Increment, 1000) {
...
pub fn call_times_out_after_specified_time_test() {
  ...
  case dream_singleton.start(name, initial_state, no_reply_handler) {
    Ok(_pid) -> {
      case dream_singleton.call(name, GetCount, 10) {
        // Very short timeout
        Ok(_) -> should.fail()
        Error(_) -> Nil
        // Expected timeout
...
pub fn is_running_returns_false_when_not_started_test() {
  ...
  dream_singleton.is_running(name) |> should.be_false()
}
